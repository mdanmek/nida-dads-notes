...
**คำอธิบายเพิ่มเติม:** user data ไม่ไหลผ่าน NameNode; NameNode ให้ metadata แล้ว client อ่าน/เขียนกับ DataNode โดยตรง การทำ HA ใช้ Active/Standby NameNodes และ shared edit mechanism ไม่ใช่ Secondary NameNode เอกสาร Apache ระบุว่า HDFS ปัจจุบันรองรับ append/truncate แต่ไม่รองรับการแก้ไขแบบสุ่มกลางไฟล์ จึงไม่เหมาะกับ OLTP ที่อัปเดต record เล็ก ๆ บ่อย
...
**ตัวอย่างคำนวณ:** ไฟล์ 300 MB เมื่อ block size = 128 MB ใช้ 3 logical blocks: 128 + 128 + 44 MB หาก replication factor = 3 จะเก็บข้อมูล payload โดยประมาณ 900 MB (ยังไม่รวม checksum/metadata และ block สุดท้ายไม่ถูกบังคับให้กินพื้นที่เต็ม 128 MB)
...
ลำดับย่อ: client ส่ง application → RM จัด container ให้ AM → AM ขอ containers เพิ่ม → NMs เริ่ม tasks → AM ติดตาม/retry → คืนทรัพยากรเมื่อจบ

**เปรียบเทียบ:** HDFS ตอบว่า “ข้อมูลอยู่ที่ไหน” ส่วน YARN ตอบว่า “ใครได้ใช้ CPU/RAM ที่ไหนและเมื่อใด”
...
**คำอธิบายเพิ่มเติม:** อย่าจำว่า “หนึ่ง HDFS block เท่ากับหนึ่ง mapper” เสมอไป จำนวน splits ขึ้นกับ InputFormat, compression และ configuration ส่วน Reducer ไม่เห็นค่าที่มาจาก mapper ทีละเครื่อง แต่เห็นค่าที่ถูกรวมตาม key หลัง shuffle/sort แล้ว
