...
**คำอธิบายพื้นฐาน:** Workflow orchestration คือกระบวนการกำหนดและควบคุมว่า **งานย่อยใดต้องรัน เมื่อใด รอใคร ล้มแล้วทำอย่างไร และสำเร็จแล้วส่งต่ออะไร** ตัว orchestrator ไม่ใช่ processing engine: มันสั่งและเฝ้าดู SQL, MapReduce, Spark, shell หรือ API แต่ไม่ได้คำนวณแทนเครื่องมือเหล่านั้น

ตัวอย่างเล็กที่สุดคือ “ดาวน์โหลดไฟล์ → ตรวจว่าไฟล์ครบ → คำนวณยอด → ส่ง dashboard” ถ้าการตรวจไฟล์ล้ม ขั้นคำนวณต้องไม่เริ่ม; ถ้าส่ง dashboard ล้มอาจ retry เฉพาะขั้นสุดท้ายโดยไม่คำนวณซ้ำทั้งหมด Input ของ orchestrator คือคำจำกัดความ tasks, dependencies, schedule และ policies ส่วน output คือ task runs, states, logs และการ trigger งานปลายทาง

ในภาษาทั่วไป orchestrator คล้ายผู้จัดเวทีที่เช็กว่านักแสดงและฉากพร้อมก่อนเปิดม่าน แต่ไม่ใช่นักแสดงเอง เมื่อเข้าสู่ศัพท์ทางการ งานย่อยเรียก **task**, การรันหนึ่งรอบเรียก **workflow/DAG run**, ความสัมพันธ์ “B รอ A” เรียก **dependency**, และสถานะอาจเป็น success, failed, retry หรือ skipped
