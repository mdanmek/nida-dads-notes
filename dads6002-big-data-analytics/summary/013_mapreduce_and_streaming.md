...
#### ตัวอย่างเล็กที่สุดก่อนลงรายละเอียด

สมมติมี Input สอง Records คือ `cat runs` และ `cat sleeps` Mapper สอง Tasks สามารถอ่านคนละ Record แล้วสร้าง Intermediate Key-value Pairs ได้แก่ `(cat, 1)`, `(runs, 1)` และ `(sleeps, 1)` จากนั้น Shuffle จะรวบรวมค่าที่มี Key เดียวกัน เช่น `cat → [1, 1]` ก่อนส่งให้ Reducer รวมเป็น `cat → 2` ส่วน Key อื่นได้ผลเป็น `runs → 1` และ `sleeps → 1`

ภาพรวมจึงเป็น **กระจายการอ่านและสร้างผลย่อย → จัดกลุ่มผลที่มี Key เดียวกัน → สรุปแต่ละกลุ่ม** ตัวอย่างนี้ใช้ศัพท์จริงตั้งแต่ครั้งแรก แต่ยังติดตามได้จากข้อมูลเพียงสองบรรทัด

จากตัวอย่างนี้ให้แยกสิ่งที่ผู้เขียนโปรแกรมกำหนดออกจากสิ่งที่ Framework ทำให้ ผู้เขียนกำหนดว่า Mapper แปลงหนึ่ง Input Record เป็น Key-value Pairs อย่างไร และกำหนดว่า Reducer รวม Values ของ Key เดียวกันอย่างไร ส่วน Framework รับผิดชอบแบ่ง Input เป็น Tasks เริ่ม Process บน Worker ส่งและจัดกลุ่ม Intermediate Data และเริ่ม Task ใหม่เมื่อเกิด Failure หากจำเพียงว่า “Map ก่อน Reduce” เราจะยังไม่เห็นว่าคุณค่าหลักของ MapReduce อยู่ที่การจัดการงานแบบกระจายรอบฟังก์ชันสองส่วนนี้
...
| Intermediate result | ผลชั่วคราวหลัง Map ซึ่งยังไม่ใช่คำตอบสุดท้าย | `(cat,1),(runs,1),(cat,1),(sleeps,1)` |
...
เมื่อเห็นเส้นทางจาก Input Records ไปสู่ผลนับคำแล้ว เราจึงแยกหน้าที่ของ Component แต่ละส่วนอย่างเป็นทางการดังนี้
