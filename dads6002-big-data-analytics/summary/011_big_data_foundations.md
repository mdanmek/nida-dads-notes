...
> **จากเอกสาร:** [dads6002_01_hadoop.pdf](../lecture/dads6002_01_hadoop.pdf) หน้า 1–10  
...
ให้มอง 5Vs เป็นคำถามออกแบบที่ต่อเนื่องกัน ไม่ใช่ป้ายห้าคำที่แยกจากกัน **Volume** ถามว่าปริมาณสะสมใหญ่เพียงใดและเครื่องเดียวเก็บหรือสแกนทันหรือไม่ ถ้าไม่ทัน เราอาจต้องกระจาย storage และ computation แต่การเพิ่มเครื่องไม่ได้ตอบ **Velocity** ซึ่งถามต่อว่าข้อมูลมาถึงเร็วเพียงใดและธุรกิจยอมรอคำตอบได้นานเท่าไร รายงานสิ้นวันกับการเตือนทุจริตภายในห้าวินาทีอาจมีปริมาณข้อมูลเท่ากัน แต่ต้องใช้วิธีประมวลผลต่างกันเพราะเวลาที่อนุญาตต่างกัน

เมื่อข้อมูลมาจากฐานข้อมูล JSON ภาพ และ log พร้อมกัน **Variety** จะบังคับให้เราตัดสินใจว่าจะรักษารูปดิบไว้ตรงไหนและสร้างโครงสร้างมาตรฐานเมื่อใด จากนั้น **Veracity** ถามว่าแต่ละแหล่งเชื่อถือได้เพียงใด เช่น GPS อาจคลาดเคลื่อนหรือธุรกรรมอาจส่งซ้ำ ถ้าไม่สร้างกฎตรวจคุณภาพ ความเร็วและปริมาณที่เพิ่มขึ้นจะเพียงทำให้เราผลิตคำตอบผิดได้เร็วขึ้น สุดท้าย **Value** ถามว่าผลลัพธ์เปลี่ยนการตัดสินใจอะไร หากไม่มีผู้ใช้ การตัดสินใจ หรือผลลัพธ์ที่วัดได้ โครงการก็อาจเป็นเพียงคลังข้อมูลราคาแพง

ตัวอย่างร้านยาแสดงความสัมพันธ์นี้ได้ชัด รายการขายหลายปีสร้าง Volume, ข้อมูลหน้าร้านที่เข้าต่อเนื่องสร้าง Velocity, ธุรกรรมกับภาพใบเสร็จสร้าง Variety และรหัสสินค้าหายหรือยอดติดลบสร้าง Veracity ปัญหาเหล่านี้มีเหตุผลให้ลงทุนก็ต่อเมื่อ Value เช่น ลดสินค้าขาด ลดความสูญเสีย หรือค้นหารายการผิดปกติ มีความสำคัญพอ ดังนั้นคำตอบข้อสอบที่ดีไม่ควรเพียงจับคู่เหตุการณ์กับตัว V แต่ควรบอกผลต่อการออกแบบและคุณค่าที่ต้องการด้วย
...
คำว่า “ผลิตภัณฑ์” ในที่นี้ไม่ได้หมายความว่าต้องขายให้ลูกค้าภายนอก จุดสำคัญคือมีผู้ใช้และมีการตัดสินใจที่เกิดซ้ำ เช่น ระบบแนะนำสินค้าที่เลือกข้อเสนอให้ลูกค้า หรือระบบเติมสต็อกที่เสนอจำนวนสั่งซื้อให้เภสัชกร ข้อมูลเป็น input, กฎหรือโมเดลเปลี่ยนข้อมูลให้เป็นคำแนะนำ, ผู้ใช้ตอบสนองต่อคำแนะนำ และผลจริงย้อนกลับมาเป็นข้อมูลรอบใหม่ วงจรนี้อธิบายข้อความในสไลด์ที่ว่า data product ใช้ข้อมูลเพื่อสร้างคุณค่าและผลิตข้อมูลกลับมาเพิ่ม

Data Science จึงไม่ใช่ชื่อของขั้นสร้างโมเดลเพียงขั้นเดียว สถิติช่วยแยกสัญญาณออกจากความผันผวน Machine Learning ช่วยเรียนรู้รูปแบบ Database Management ทำให้ข้อมูลค้นหาและควบคุมได้ Software Engineering ทำให้สิ่งที่ทดลองกลายเป็นระบบที่ทดสอบและดูแลได้ และ Distributed Computing ทำให้งานขยายไปยังข้อมูลจำนวนมากได้ หากโมเดลแม่นใน notebook แต่รับข้อมูลจริงไม่ได้ ตรวจ version ไม่ได้ หรือส่งคำตอบไม่ทันเวลา สิ่งนั้นยังไม่เป็น data product ที่ใช้งานได้
...
|---|---|
| จำแนกประเภทข้อมูล | จัดประเภท transaction, JSON, log และภาพ พร้อมอธิบายโครงสร้าง |
| วิเคราะห์ 5Vs | โจทย์ fraud ภายใน 5 วินาทีและผลต่อ architecture |
| Trace workflow | ตัวอย่าง `TX1001` และ reconciliation ของ accepted/rejected records |
| เลือก batch/Lambda/unified | กรณี late correction และคำถาม Analyze |
| ออกแบบ Data Lake | Guided Design Lab และ metadata ของ raw zone |
